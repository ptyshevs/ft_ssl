...
def fuzz(ft_ssl_path, subcommand, ssl_binary="openssl", n=100, l=10, verbose=0):
    ...
    for _ in range(n):
        ...
        my_command = command_template.format(inp, ft_ssl_path)
        gt_command = command_template.format(inp, ssl_binary)
        ...
        if not same_result:
            error_log.append((gt_command, gt_out, my_command, my_out))
...
def output_errors(error_log, file=sys.stdout):
    ...
    for (gt_in, gt_out, my_in, my_out) in error_log:
        print("[openssl in]:{}\n[openssl_out]: {}\n[ft_ssl in]:{}\n[ft_ssl out]: {}".format(gt_in, gt_out, my_in, my_out))
...
def run_ut_suite(ft_ssl_path, subcommand, test_suite, ssl_binary="openssl", verbose=False):
    ...
    for (test_name, test_input) in test_suite:
        ...
        same_result = gt_out == my_out
        if not same_result:
            errors.append((gt_command, gt_out, my_command, my_out))
        if verbose == 1 and not same_result:
            print("[gt] {} != {} [ft]".format(gt_out, my_out))
        else:
            print("{}.{}".format(GREEN if same_result else RED, NC), end='', flush=True)
...
if __name__ == '__main__':
    ...
    ut_errors = run_ut_suite(args.path, subcommand, test_suite)
